...
echo $($PSQL "SELECT distinct(name) FROM teams AS Tm LEFT JOIN games AS W ON Tm.team_id = W.winner_id LEFT JOIN games AS O ON Tm.team_id = O.opponent_id WHERE (W.year = 2014 AND W.round = 'Eighth-Final') OR (O.year = 2014 AND O.round = 'Eighth-Final')")
